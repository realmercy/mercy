  ...
  switch (stateChange.type) {
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          .setAuthor(`Resuming Playback! ▶️`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(
            `Resuming playback because someone joined the voice channel and I like listening with my friends`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          .setAuthor(`Music Player Paused! ⏸`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(`The player has been paused because there are no users in the voice channel`);
        await client.channels.cache.get(player.textChannel).send(emb);
      }
      break;
  }
