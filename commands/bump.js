...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0])
      return client.sendTime(message.channel, "<:no:930114529478733834> | **Invalid arguments.**");

    // Check if (args[0] - 1) is a valid index
    let trackNum = parseInt(args[0] - 1);
    if (trackNum < 1 || trackNum > player.queue.length - 1) {
      return client.sendTime(message.channel, "<:no:930114529478733834> | **Invalid track number.**");
    }
    ...
    client.sendTime(
      message.channel,
      "<a:ok:930121961542144030> | **" + track.title + "** has been moved to the front of the queue."
    );
  },
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!args[0].value)
        return client.sendTime(interaction, "<:no:930114529478733834> | **Invalid track number.**");
